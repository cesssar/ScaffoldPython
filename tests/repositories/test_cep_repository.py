...
def test_create_cep_success(
    ...
    created_cep = cep_repository_instance.create_cep(
        db_session, sample_cep_data
    )
...
def test_get_cep_found(
    ...
    retrieved_cep = cep_repository_instance.get_cep(
        db_session,
        sample_cep_data["cep"]
    )
...
def test_get_cep_not_found(db_session: Session,
                           cep_repository_instance: CEPRepository
                           ):
...
def test_create_cep_database_error(
    ...
    created_cep = cep_repository_instance.create_cep(db_session,
                                                     sample_cep_data)
